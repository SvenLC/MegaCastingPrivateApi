...
exports.getClient = (req, res, next) => {
    ...
    .then(client => {
        if (!client) {
            const error = new Error({message: 'Client inexistant !'});
...
exports.createClient = (req, res, next) => {
    ...
    .then(client => {
        res.status(201).json({message: 'Client crée', client: client})        
      })
      .catch(err => {
        if (!err.statusCode) {
            err.statusCode = 500;
...
exports.deleteClient = (req, res, next) => {
    ...
    Client.findByPk(clientId)
        .then(client => {
            ...
            res.status(200).json({message: 'Client supprimé', client: client});
...
exports.updateClient = (req, res, next) => {
    ...
    .then(client => {
        ...
        res.status(200).json({message: 'Client modifié', client: client});
